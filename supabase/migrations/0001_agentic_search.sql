-- Agentic search v1 migration for ceejay

CREATE EXTENSION IF NOT EXISTS vector;
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Expression index for weighted lexical search without generated column.
CREATE INDEX IF NOT EXISTS idx_companies_search_expr_v1
ON companies USING GIN ((
  ...
  setweight(to_tsvector('english', coalesce(problem_solved, '')), 'C') ||
  setweight(to_tsvector('english', coalesce(differentiator, '')), 'C')
));

CREATE INDEX IF NOT EXISTS idx_companies_name_trgm_v1
ON companies USING GIN (company_name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_embeddings_type_company_v1
ON company_embeddings (embedding_type, company_id);
...
CREATE TABLE IF NOT EXISTS search_runs (
  ...
  status_scope TEXT[] NOT NULL DEFAULT ARRAY['startup'],
...
CREATE TABLE IF NOT EXISTS search_run_steps (
  ...
  candidate_count_before INT NOT NULL DEFAULT 0,
  candidate_count_after INT NOT NULL DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_search_runs_session ON search_runs(session_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_search_run_steps_run ON search_run_steps(run_id, iteration_no, step_order);
CREATE INDEX IF NOT EXISTS idx_search_run_results_run ON search_run_results(run_id, rank);
...
CREATE OR REPLACE FUNCTION get_companies_by_ids_v1(
  p_company_ids TEXT[]
)
